...
class TagAffinity(BaseModel):
    ...
    async def get_all(cls,
        ...
        response = await DatabaseFactory.get_all("TagAffinity", sort, filter, page, pageSize)
        
        if response["data"]:
            for user_dict in response["data"]:
                # Process Pydantic and FK validation if enabled
                if validation:
                    utils.validate_model(cls, user_dict, "TagAffinity")
                    await utils.validate_fks("TagAffinity", user_dict, cls._metadata)
                
                # Populate view data if requested
                if view_spec:
                    await utils.populate_view(user_dict, view_spec, "TagAffinity")
    ...
    async def get(cls, id: str, view_spec: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        response = await DatabaseFactory.get_by_id(str(id), "TagAffinity")
        if response["data"]:
    ...
    async def save(self, entity_id: str = '', validate: bool = True) -> tuple[Self, List[str]]:
        ...
        if validate:
            # Pydantic validation
            validated_instance = utils.validate_model(self.__class__, data, "TagAffinity")
            data = validated_instance.model_dump(mode='python')
            
            # FK validation (sends notifications, doesn't throw)
            await utils.validate_fks("TagAffinity", data, self._metadata)
        
        # Unique validation (always enforced)
        unique_constraints = self._metadata.get('uniques', [])
        if unique_constraints:
            await utils.validate_uniques("TagAffinity", data, unique_constraints, entity_id if entity_id.strip() else None)

        # Save document
        response = await DatabaseFactory.save("TagAffinity", data, entity_id)
    ...
    @classmethod
    async def delete(cls, tagaffinity_id: str) -> bool:
        return await DatabaseFactory.delete("TagAffinity", tagaffinity_id)
