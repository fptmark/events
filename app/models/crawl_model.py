...
class Crawl(BaseModel):
    ...
    async def get_all(cls,
        ...
        response = await DatabaseFactory.get_all("Crawl", sort, filter, page, pageSize)
        
        if response["data"]:
            for user_dict in response["data"]:
                # Process Pydantic and FK validation if enabled
                if validation:
                    utils.validate_model(cls, user_dict, "Crawl")
                    await utils.validate_fks("Crawl", user_dict, cls._metadata)
                
                # Populate view data if requested
                if view_spec:
                    await utils.populate_view(user_dict, view_spec, "Crawl")
    ...
    async def get(cls, id: str, view_spec: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        response = await DatabaseFactory.get_by_id(str(id), "Crawl")
        if response["data"]:
    ...
    async def save(self, entity_id: str = '', validate: bool = True) -> tuple[Self, List[str]]:
        ...
        if validate:
            # Pydantic validation
            validated_instance = utils.validate_model(self.__class__, data, "Crawl")
            data = validated_instance.model_dump(mode='python')
            
            # FK validation (sends notifications, doesn't throw)
            await utils.validate_fks("Crawl", data, self._metadata)
        
        # Unique validation (always enforced)
        unique_constraints = self._metadata.get('uniques', [])
        if unique_constraints:
            await utils.validate_uniques("Crawl", data, unique_constraints, entity_id if entity_id.strip() else None)

        # Save document
        response = await DatabaseFactory.save("Crawl", data, entity_id)
    ...
    @classmethod
    async def delete(cls, crawl_id: str) -> bool:
        return await DatabaseFactory.delete("Crawl", crawl_id)
