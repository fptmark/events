...
class Profile(BaseModel):
    ...
    async def get_all(cls) -> Dict[str, Any]:
        try:
            ...
            if get_validations:
                for doc in raw_docs:
                    try:
                        profiles.append(cls.model_validate(doc))
                    except PydanticValidationError as e:
                        # Convert Pydantic errors to notifications
                        entity_id = doc.get('id')
                        if not entity_id:
                            notify_warning("Document missing ID field", NotificationType.DATABASE, entity=Profile)
                            entity_id = "missing"
                        ...
                        profiles.append(cls.model_construct(**doc))
            else:
                profiles = [cls.model_construct(**doc) for doc in raw_docs]  # NO validation  
                ...
                with python_warnings.catch_warnings(record=True) as caught_warnings:
                    ...
                    if caught_warnings:
                        ...
                        if datetime_field_names:
                            ...
                            notify_warning(f"{field_list} datetime serialization warnings", NotificationType.VALIDATION, entity="Profile", entity_id=entity_id)
                        else:
                            # Fallback for non-datetime warnings
                            warning_count = len(caught_warnings)
                            notify_warning(f"User {entity_id}: {warning_count} serialization warnings", NotificationType.VALIDATION, entity="Profile")
            ...
            if get_validations:
                try:
                    ...
                    for error in e.errors():
                        field_name = str(error['loc'][-1])
                        notify_warning(
                            message=f"Profile {entity_id}: {field_name} validation failed - {error['msg']}",
                            type=NotificationType.VALIDATION,
                            entity="Profile",
                            field_name=field_name,
                            value=error.get('input'),
        ...
        try:
            ...
            try:
                ...
                for err in e.errors():
                    ...
                    notify_warning(
                        message=f"Profile {entity_id}: {field_name} validation failed - {err['msg']}",
                        type=NotificationType.VALIDATION,
                        entity="Profile",
                        field_name=field_name,
                        value=err.get("input"),
                ...
                raise ValidationError(message="Validation failed before save", entity="Profile", invalid_fields=failures)
    ...
    @classmethod
    async def delete(cls, profile_id: str) -> tuple[bool, List[str]]:
        try:
            result = await DatabaseFactory.delete_document("profile", profile_id)
            if not result:
