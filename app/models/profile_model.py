...
class Profile(BaseModel):
    ...
    async def get_all(cls) -> tuple[Sequence[Self], List[ValidationError]]:
        try:
            ...
            raw_docs, warnings = await DatabaseFactory.get_all("profile", unique_constraints)
            ...
            if get_validations:
                for doc in raw_docs:
                    try:
                        ...
                        profiles.append(cls(**doc))
            else:
                profiles = [cls(**doc) for doc in raw_docs]  # NO validation
            
            # Add database warnings to validation errors
            validation_errors.extend([ValidationError(message=w, entity="Profile", invalid_fields=[]) for w in warnings])
