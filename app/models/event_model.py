            ...
            if get_validations:
                for doc in raw_docs:
                    try:
                        ...
                        if not entity_id:
                            notify_warning("Document missing ID field", NotificationType.DATABASE, entity=Event)
                            entity_id = "missing"
                ...
                with python_warnings.catch_warnings(record=True) as caught_warnings:
                    ...
                    if caught_warnings:
                        ...
                        if datetime_field_names:
                            ...
                            notify_warning(f"{field_list} datetime serialization warnings", NotificationType.VALIDATION, entity="Event", entity_id=entity_id)
                        else:
                            # Fallback for non-datetime warnings
                            warning_count = len(caught_warnings)
                            notify_warning(f"User {entity_id}: {warning_count} serialization warnings", NotificationType.VALIDATION, entity="Event")
