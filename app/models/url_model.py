    ...
    async def get_all(cls) -> Dict[str, Any]:
        try:
            ...
            if get_validations:
                for doc in raw_docs:
                    try:
                        urls.append(cls.model_validate(doc))
                    except PydanticValidationError as e:
                        # Convert Pydantic errors to notifications
                        entity_id = doc.get('id')
                        if not entity_id:
                            notify_warning("Document missing ID field", NotificationType.DATABASE, entity=Url)
                            entity_id = "missing"
            ...
            for url in urls:
                with python_warnings.catch_warnings(record=True) as caught_warnings:
                    ...
                    data_dict = url.model_dump()
                    url_data.append(data_dict)
                    ...
                    if caught_warnings:
                        ...
                        if datetime_field_names:
                            ...
                            notify_warning(f"{field_list} datetime serialization warnings", NotificationType.VALIDATION, entity="Url", entity_id=entity_id)
                        else:
                            # Fallback for non-datetime warnings
                            warning_count = len(caught_warnings)
                            notify_warning(f"User {entity_id}: {warning_count} serialization warnings", NotificationType.VALIDATION, entity="Url")
