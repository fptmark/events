from fastapi import APIRouter, HTTPException
from typing import List
from app.models.user_model import User
from app.utils.db import get_db
from bson import ObjectId

router = APIRouter()

# CRUD Operations
@router.post('/', response_model=User)
async def create_user(item: User):
    db = get_db()
    item_dict = item.dict()
    item_dict['_id'] = str(ObjectId())
    await db['users'].insert_one(item_dict)
    return item_dict

@router.get('/', response_model=List[User])
async def get_all_users():
    db = get_db()
    items = await db['users'].find().to_list(None)
    return items

@router.get('/{item_id}', response_model=User)
async def get_user(item_id: str):
    db = get_db()
    item = await db['users'].find_one({'_id': ObjectId(item_id)})
    if not item:
        raise HTTPException(status_code=404, detail='User not found')
    return item

@router.put('/{item_id}', response_model=User)
async def update_user(item_id: str, item: User):
    db = get_db()
    result = await db['users'].update_one({'_id': ObjectId(item_id)}, {'$set': item.dict()})
    if not result.matched_count:
        raise HTTPException(status_code=404, detail='User not found')
    return item

@router.delete('/{item_id}')
async def delete_user(item_id: str):
    db = get_db()
    result = await db['users'].delete_one({'_id': ObjectId(item_id)})
    if not result.deleted_count:
        raise HTTPException(status_code=404, detail='User not found')
    return {'message': 'User deleted successfully'}
